...
-- Verificar estrutura
SHOW COLUMNS FROM fila_impressao;
...
SELECT * FROM usuarios WHERE tipo = 'dono';
...
INSERT IGNORE INTO produtos (nome, descricao, preco, categoria, disponivel) VALUES
('X-Burger', 'Hambúrguer tradicional com queijo, alface e tomate', 15.90, 'Hambúrguer', 1),
('X-Bacon', 'Hambúrguer com bacon crocante e queijo cheddar', 18.90, 'Hambúrguer', 1),
('X-Egg', 'Hambúrguer com ovo frito e queijo', 17.90, 'Hambúrguer', 1),
('X-Salada', 'Hambúrguer completo com salada', 19.90, 'Hambúrguer', 1),
('X-Tudo', 'Hambúrguer com tudo que você imaginar', 24.90, 'Hambúrguer', 1),
('Coca-Cola Lata', 'Refrigerante 350ml', 5.00, 'Bebida', 1),
('Guaraná Lata', 'Refrigerante 350ml', 5.00, 'Bebida', 1),
('Suco Natural', 'Suco de laranja natural 300ml', 8.00, 'Bebida', 1),
('Batata Frita', 'Porção de batata frita crocante', 12.00, 'Acompanhamento', 1),
('Onion Rings', 'Anéis de cebola empanados', 14.00, 'Acompanhamento', 1);

-- Verificar produtos inseridos
SELECT id, nome, preco, categoria FROM produtos ORDER BY categoria, nome;
...
UNION ALL
SELECT 'carrinho', COUNT(*) FROM carrinho
UNION ALL
SELECT 'fila_impressao', COUNT(*) FROM fila_impressao
UNION ALL
...
WHERE TABLE_SCHEMA = 'burger_house'
...
WHERE ip.id_pedido = 1;  -- Altere o ID do pedido
...
SELECT COUNT(*) FROM usuarios WHERE tipo = 'dono';
