...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['adicionar'])) {
    ...
    $sql = "INSERT INTO produtos_especiais (nome, preco, descricao, imagem) VALUES ('$nome', '$preco', '$descricao', '$imagem')";
    $conn->query($sql);
    header("Location: cardapio_especial.php");
    exit;
}

// Excluir produto especial
if (isset($_GET['delete'])) {
    $id = $_GET['delete'];
    $res = $conn->query("SELECT imagem FROM produtos_especiais WHERE id=$id");
    $row = $res->fetch_assoc();
    if ($row['imagem'] && file_exists($targetDir . $row['imagem'])) {
        unlink($targetDir . $row['imagem']);
    }
    $conn->query("DELETE FROM produtos_especiais WHERE id=$id");
    header("Location: cardapio_especial.php");
    exit;
}

$produtos = $conn->query("SELECT * FROM produtos_especiais");
?>
...
<head>
    ...
    <style>
        /* Customizações específicas para cardápio especial */
        .card-produto {
...
<body>
    ...
    <form method="post" enctype="multipart/form-data">
        <input type="text" name="nome" placeholder="Nome do produto especial" required>
    ...
    <div class="produtos-container">
        <?php while ($row = $produtos->fetch_assoc()): ?>
            <div class="card-produto">
                ...
                <?php endif; ?>
                <div class="card-info">
                    <h3><i class="fa fa-star"></i> <?= $row['nome'] ?></h3>
